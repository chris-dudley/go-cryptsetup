...
func (d *Device) keyslotAddByVolumeKey(keyslot int, volumeKey []byte, newPassphrase []byte) (out int, err error) {
	arglist := (*C.struct_gocrypt_logstack)(nil)

	// not a pointer
	_keyslot := (C.int)(keyslot)

	_volumeKey := unsafe.Pointer(nil)
	if volumeKey != nil {
		_volumeKey = C.CBytes(volumeKey)
		defer C.free(_volumeKey)
	} else {
		// this value can be nil
	}

	// not a pointer
	_volumeKeySize := (C.size_t)(len(volumeKey))

	_new_passphrase := unsafe.Pointer(nil)
	if newPassphrase != nil {
		_new_passphrase = C.CBytes(newPassphrase)
	...
	// not a pointer
	_new_passphrase_size := (C.size_t)(len(newPassphrase))
